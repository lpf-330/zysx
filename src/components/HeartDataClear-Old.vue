<template>
    <div class="chart-container">
        ...
        <ModernHealthAlert v-if="analysisResult && analysisResult.summary" :analysis-result="analysisResult" data-type="heart"
        ...
        if (apiResponse && apiResponse.code === 200 && Array.isArray(responseData) && responseData.length > 0) {
            ...
            nextTick(() => {
                try {
                    ...
                    console.warn("分析健康数据时出错（组件可能已卸载）:", e);
                }
            });
        } else {
...
const analyzeHealthData = (dataPoints, timePoints) => {
    if (!isMountedFlag) {
        console.warn("HeartDataClear 组件已卸载，跳过健康数据分析");
        return;
    }
    ...
    try {
        const result = healthAnalyzer.HeartRateRules.analyzeHeartRate(dataPoints, timePoints);
        // 再次检查组件状态后再赋值
        try {
            if (isMountedFlag) {
...
const debouncedFetchData = () => {
    if (!isMountedFlag) {
        console.log("HeartDataClear: 组件已卸载，跳过防抖数据获取");
        return;
    }
    if (fetchTimeout) {
...
onBeforeUnmount(() => {
    ...
    console.log("=== HeartDataClear.vue 组件卸载完成 ===");
});
</script>
