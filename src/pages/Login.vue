...
function handleCardMouseMove(e) {
  ...
  card.style.boxShadow = `${(centerX - x) / 5}px ${(centerY - y) / 5}px 30px rgba(0,0,0,0.15)`
}

function handleCardMouseLeave(e) {
  const card = e.currentTarget
  card.style.transform = 'perspective(1000px) rotateX(0deg) rotateY(0deg) scale(1)'
  card.style.boxShadow = '0 20px 35px -12px rgba(0, 0, 0, 0.08)'
}
  ...
  <div class="login-modal" :style="{ display: modalDisplay }">
    <div class="modal-card" @click.stop>
      <div class="modal-header">
        ...
        <span class="close-modal" @click="modalDisplay = 'none'">&times;</span>
      </div>
      ...
      <div class="linkBox">
        <router-link v-if="loginType === 'user'" to="/register" class="link">还没有账户？立即注册</router-link>
        <a v-else href="#" @click.prevent="loginType = 'user'" class="link">切换到用户端登录</a>
      </div>
    </div>
  </div>
</template>
...
.hero-section::before {
  ...
  right: 0;
  bottom: 0;
  background: linear-gradient(135deg, rgba(217, 234, 211, 0.85) 0%, rgba(194, 221, 185, 0.85) 40%, rgba(170, 207, 161, 0.85) 100%);
  z-index: 1;
  pointer-events: none;
}
